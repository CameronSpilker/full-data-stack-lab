...
The site is static, built from the published warehouse and hosted on Vercel at
lab.cameronspilker.com. One deployment carries three surfaces, each under its
own prefix, plus a landing page that links to the two dashboards. The Evidence
dashboard and the dbt Charts boards do not link to each other.

| Surface              | Path          | Source |
| -------------------- | ------------- | ------ |
| Landing page         | `/`           | `dashboard/landing/` |
| Evidence dashboard   | `/evidence/`  | `dashboard/pages/` (built with `basePath: /evidence`) |
| dbt Charts boards    | `/charts/`    | `charts/`, rendered by the pipeline |
| dbt docs             | `/docs/`      | generated by the pipeline |

`dashboard/scripts/assemble-site.sh` lays these out after the Evidence build.
Old root-level Evidence URLs (`/rankings`, `/teams/...` and so on) redirect to
`/evidence/...` through `dashboard/vercel.json`.
...
The build command fetches the warehouse, the docs and the boards from the
release below, runs the normal Evidence build, then assembles the site. Every push redeploys.
